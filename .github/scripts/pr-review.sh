...
if grep -qiE '\.github/workflows/|\.github/scripts/|Cargo\.(toml|lock)|CODEOWNERS|(^|["/])migrations/|(^|["/])proto/|billing|metering|credits|ledger|rbac|crypto|byok|byom|tenant_isolation|hmac|(^|["/])audit|secrets|keycloak|(^|["/])auth|installer/|release_channel|license\.rs|preflight\.rs|v1-surface-contract|(^|["/])(clean|purge|bulk-delete|wipe)-' <<<"$FILES"; then
  HIGH_RISK=1
fi
